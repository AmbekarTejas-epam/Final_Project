...
def run_inference():
    ...
    try:
        ...
        logger.info("Loaded inference data. Shape: %s", df.shape)
        mlflow.log_param("inference_dataset_size", df.shape[0])

        if "review" not in df.columns:
            raise ValueError("Input data must contain a 'review' column.")

        reviews = df["cleaned"].astype(str)

        # Load model and vectorizer and SVD
        model_path = "/app/outputs/models/logreg_model.pkl"
        vec_path = "/app/outputs/models/tfidf.pkl"
        svd_path = "/app/outputs/models/svd.pkl"

        if not os.path.exists(model_path) or not os.path.exists(vec_path) or not os.path.exists(svd_path):
            raise FileNotFoundError("Model, vectorizer, or SVD transformer not found.")

        model = joblib.load(model_path)
        vectorizer = joblib.load(vec_path)
        svd = joblib.load(svd_path)

        # Vectorize and apply SVD
        ...
        logger.info("Inference completed in %.2fs", duration)
